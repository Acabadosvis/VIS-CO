...
// ── Reemplazar en DOCX (XML directo)
function replaceInDocx(docxBuffer, replacements) {
  // El DOCX es un ZIP - usamos Buffer para manipular el XML interno
  // Necesitamos extraer el document.xml, hacer reemplazos, y reempacar
    ...
    return zip.file('word/document.xml').async('string').then(xml => {
      ...
      for (const [old, nw] of items) {
        ...
        const escaped = String(nw)
          ...
          .replace(/</g, '&lt;')
          .replace(/>/g, '&gt;');
        // Reemplazar en el XML (puede estar partido en runs, así que buscamos en texto plano también)
        const escapedOld = old.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        newXml = newXml.split(escapedOld).join(escaped);
      }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { tipo, nombre, cedula, lugar_exp, direccion, cotizacion, valor_total, congelacion, fecha_cong, dia, mes, torre, apto, proyecto } = datos;

    const minutaPath = path.join(__dirname, '../../minutas', `${tipo}.docx`);
    const docxBuffer = fs.readFileSync(minutaPath);

    const vt = valor_total;
    const cong = congelacion || 5000000;
